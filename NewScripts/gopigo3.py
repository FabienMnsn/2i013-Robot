...
class GoPiGo3(object):
    ...
    def grove_i2c_start(self, port, addr, outArr, inBytes=0):
        ...
        if inBytes > self.GROVE_I2C_LENGTH_LIMIT:
            raise RuntimeError(
                "Read length error. Up to %d bytes can be read in a single transaction." % self.GROVE_I2C_LENGTH_LIMIT)

        outBytes = len(outArr)
        if outBytes > self.GROVE_I2C_LENGTH_LIMIT:
            raise RuntimeError(
                "Write length error. Up to %d bytes can be written in a single transaction." % self.GROVE_I2C_LENGTH_LIMIT)
